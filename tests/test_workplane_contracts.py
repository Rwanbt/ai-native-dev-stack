...
class WorkPlaneContractsTests(unittest.TestCase):
    ...
    def test_valid_minimal_artifacts_cover_all_pr01_schema_identities(self):
        values = [
            ...
            artifact("verification_run", uid=uid("run"), work=reference("work"), contract_revision=1, contract_digest=DIGEST, verification_specification=reference("verify"), command_registry_digest=DIGEST, policy_digest=DIGEST, approval_root=reference("root"), repository_snapshot=reference("snapshot"), producer="runtime", producer_version="1.0", evidence_provenance="CI_APPROVED", command="check", result="PASS", exit_code=0, started_at="2026-09-02T00:00:00Z", finished_at="2026-09-02T00:00:01Z", duration_ms=1000, stdout_digest=DIGEST, stderr_digest=DIGEST, substance_metadata={}),
