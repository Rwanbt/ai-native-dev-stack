#!/usr/bin/env python3
"""Machine-level ownership: what the global installer wrote, and its reversal.

`scripts/install_agents.py` copies blocks into harness files, links skills into
`~/.agents` and `~/.claude`, and renders two plugin files. Until now nothing
recorded *what* it had written, so the only available uninstall was manual and
the user had no way to tell an AI Native file from their own.

This module owns one manifest - `~/.ai-native/machine.json` - and one reversal
rule:

* a **link** is ours when it resolves to the recorded source; removing it never
  touches the source (a symlink or junction is removed, not followed);
* a **rendered file** is ours when its bytes still hash to the recorded digest;
  a file the user edited is preserved and reported;
* a **block** is ours while its markers are present (the markers are the
  ownership proof, as everywhere else in this stack); user text around the
  block is preserved, and a file that becomes empty is removed.

A file we never wrote is never touched, and a file we wrote that the user has
since changed is preserved, not deleted. The manifest itself is deleted only
...
def save(home: Path, *, version: str, assets: list[dict]) -> Path:
    path = manifest_path(home)
    path.parent.mkdir(parents=True, exist_ok=True)
    record = {
        "schema_version": SCHEMA_VERSION,
        "installed_by": "scripts/install_agents.py",
...
__all__ = ["SCHEMA_VERSION", "MANIFEST_RELATIVE", "MachineLifecycleError",
           "manifest_path", "load", "save", "uninstall", "digest_file"]