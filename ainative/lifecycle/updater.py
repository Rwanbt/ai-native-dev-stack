...
def _distribution_root(extracted: Path, release: providerlib.Release | None = None,
    ...
    under the directory that document names. The protocol version and the
    release version are validated before the payload is trusted; a bundle that
    names another release is refused here as well as at the internal-VERSION
    gate, because the two files are written by different steps and only one of
    them can be regenerated by a stale cache.

    A bare payload at the archive root stays accepted: that is the layout
    fixtures and direct downloads use.
    """

    manifest = extracted / providerlib.PROTOCOL_MANIFEST
    if manifest.is_file():
        try:
            document = json.loads(manifest.read_text(encoding="utf-8"))
        except (OSError, ValueError) as error:
            raise LifecycleError("UPDATE_INTEGRITY_FAILED",
                                 f"{providerlib.PROTOCOL_MANIFEST} is unreadable: {error}") from error
        ...
        if release is not None and declared != release.version:
            raise LifecycleError(
                "UPDATE_VERSION_MISMATCH",
                f"bundle declares release_version {declared!r} but the release says "
                f"{release.version!r}",
                release_version=release.version, bundle_version=declared)
        payload_root = document.get("payload_root")
        if not isinstance(payload_root, str) or not payload_root:
            raise LifecycleError("UPDATE_INTEGRITY_FAILED",
                                 "bundle declares no payload_root")
        try:
            relative = validate_relative(payload_root)
        except LifecycleError as error:
            raise LifecycleError("UPDATE_INTEGRITY_FAILED",
                                 f"bundle payload_root is not a safe path: {error.message}") from error
        payload = extracted.joinpath(*relative.parts)
        if not (payload / "VERSION").is_file():
            raise LifecycleError("UPDATE_INTEGRITY_FAILED",
                                 f"bundle payload root {payload_root!r} holds no VERSION")
        return payload
