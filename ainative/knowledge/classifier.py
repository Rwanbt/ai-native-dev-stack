...
The classifier proposes; promotion never happens solely because the
classifier said so (INV-04). `suggest` is a pure keyword heuristic with
stated reasons — a semantic model may replace the heuristic later, but
the output contract (kind plus reasons, advisory only) does not change.
"""
