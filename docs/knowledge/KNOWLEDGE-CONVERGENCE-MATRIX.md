...
| 2 | Advisory classification (kind + reasons) | classifier.py (43 l) | none | MISSING | port as pure keyword heuristic; advisory-only contract; versioned categories; UNKNOWN never guessed |
...
| 7 | Cross-harness import (preview/apply, staged as candidates) | imports.py (149 l) | none | MISSING | port preview/apply staging; never canonical direct import; preserve harness provenance |
...
| 18 | Working memory (bounded fields, TTL, crash-safe) | working.py (330 l) | continuity.py (K3) | PARTIAL | extend continuity; CLI `ainative context save/status/checkpoint/restore/clear`; no raw CoT |
