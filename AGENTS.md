...
Measured 2026-09-12 (distribution & lifecycle v1, Knowledge K1-K4 + Auto-Memory convergence, Multi-Vault V1) via `git ls-files` (image excluded). Re-measure
with `python3 scripts/measure_scope.py`; CI fails when these figures drift.

| Scope | Tokens (÷4) | Files | Strategy |
|---|---|---|---|
| Core stack (excl. anti-debt) | ~719 416 | 433 | **Layered read** — cartography first  then targeted reads |
| Anti-debt agent | ~129 650 | 118 | Read its `AI_CONTEXT.md` and ADRs before its sources |
| Whole repo | ~849 066 | 551 | **Multi-phase workflow** — never a single direct read |
