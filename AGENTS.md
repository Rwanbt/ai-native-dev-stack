...
Measured 2026-09-02 via `git ls-files` (image excluded). Re-measure with
`python3 scripts/measure_scope.py`; CI fails when these figures drift.

| Scope | Tokens (÷4) | Files | Strategy |
|---|---|---|---|
| Core stack (excl. anti-debt) | ~178 601 | 122 | **Layered read** — cartography first, then targeted reads |
| Anti-debt agent | ~129 648 | 118 | Read its `AI_CONTEXT.md` and ADRs before its sources |
| Whole repo | ~308 250 | 240 | **Multi-phase workflow** — never a single direct read |

Do **not** read the whole repo in one pass: at ~308k tokens it does not fit,
